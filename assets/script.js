...
// --- Smart URL Routing for Date-based Filtering ---
...
function setupBlogFilters() {
  ...
  posts.forEach(post => {
    ...
    if (year) {
      ...
      const postYear = postDate.match(/\b(\d{4})\b/)?.[1];
      if (postYear !== year) show = false;
    }
    
    // Month filter
    if (month && show) {
      const postDate = post.querySelector('.post-date')?.textContent || '';
      const postMonth = postDate.match(/\b(January|February|March|April|May|June|July|August|September|October|November|December)\b/)?.[1];
      const monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
      const targetMonth = monthNames[parseInt(month) - 1];
      if (postMonth !== targetMonth) show = false;
    }
    
    // Tag filter
    if (tag && show) {
      const postTags = post.querySelector('.post-tags')?.textContent || '';
      if (!postTags.includes(tag)) show = false;
    }
  ...
  noResults.innerHTML = `
    ...
    <p>Try adjusting your filters or <a href="/blog/">view all posts</a></p>
  `;
  
  const blogContainer = document.querySelector('.blog-container');
  if (blogContainer && visibleCount === 0) {
    blogContainer.appendChild(noResults);
  }
  
  // Update page title to reflect filters
  let title = 'Blog';
  const monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
  
  if (year && month) {
    title = `Blog - ${monthNames[parseInt(month) - 1]} ${year}`;
  } else if (year) {
    title = `Blog - ${year}`;
  } else if (tag) {
    title = `Blog - ${tag}`;
